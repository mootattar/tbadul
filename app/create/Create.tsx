...
export default function CreatePost() {
  ...
  // تعديل نوع choice لإضافة خيار "sale"
  ...
  // إضافة حالة لحفظ السعر
  const [price, setPrice] = useState("");
  // تحديث حالة الأخطاء لإضافة خطأ السعر
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    let errorsObj: { title: string; uploading: string; price: string } = {
      title: "",
      uploading: "",
      price: "",
    };
    ...
    if (errorsObj.title || errorsObj.uploading || errorsObj.price) {
      setErrors(errorsObj);
      titleRef.current?.focus();
      return;
    }
